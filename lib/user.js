...
exports.recent = function (req, callback) {
  jamon[req.session.userHash].getChats(false, true, function (err, chats) {
    if (err) {
      callback(err);
    } else {
      callback(null, chats);
...
exports.sendMessage = function (sender, receiver, message, chat, callback) {
  setUser(receiver);
  setUser(sender);
...
exports.sendNotification = function (req, recipient, io, newChat, callback) {
  ...
  jamon[recipient].isFollowing(req.session.userHash, function (err) {
    if (err) {
      callback(err);
    } else {
